<!--
  Overview: Workshop item browser for the authenticated "mods" step.
  Responsibility: Renders filters, empty states, and selectable workshop item cards that lead into update flows.
-->
...
const resolvedEmptyStateMessage = computed(() => {
  const explicitMessage = props.emptyStateMessage?.trim()
  if (explicitMessage) {
...
<template>
  <section class="mt-5" aria-label="Workshop items section">
    <article class="fade-in app-panel rounded-2xl border border-slate-200 bg-white p-5 shadow-md">
      <div class="flex flex-col gap-3">
        <div>
          ...
          <p class="mt-1 text-sm text-slate-600">Click an item to load it into the update flow.</p>
        </div>
        <div class="flex w-full flex-wrap items-center gap-2">
          <select
            :value="visibilityFilter"
            class="workshop-visibility-filter-select min-[720px]:w-[170px] rounded border border-slate-300 px-2 py-2 text-xs"
      ...
      <div
        v-if="showEmptyState && workshopItems.length === 0"
        class="mt-4 rounded-lg border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-600"
      >
        {{ resolvedEmptyStateMessage }}
      </div>

      <div v-else-if="workshopItems.length > 0" class="workshop-grid mt-4">
        <button
          ...
          :class="
            selectedWorkshopItemId === item.publishedFileId
        ...
        >
          ...
          <div class="workshop-card-details p-3">
            ...
            <div class="mt-2">
              <span
                class="workshop-visibility-badge inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold"
