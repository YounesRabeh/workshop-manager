<!--
  Overview: Viewer for the current SteamCMD session log.
  Responsibility: Shows the current session entry and its latest output lines.
-->
<script setup lang="ts">
import type { PersistedRunLog } from '@shared/contracts'

defineProps<{
  runLogs: PersistedRunLog[]
  ...
  <section class="mt-5">
    <article class="fade-in app-panel rounded-2xl border border-slate-200 bg-white p-5 shadow-md">
      <div class="flex items-center justify-between gap-2">
        <h2 class="text-lg font-semibold text-slate-800">SteamCMD Session Log</h2>
      ...
      <div class="mt-4 grid gap-4 lg:grid-cols-[280px,1fr]">
        <aside class="space-y-2">
          <button v-for="run in runLogs" :key="run.runId" class="w-full rounded-lg border px-3 py-2 text-left text-sm transition" :class="selectedRunId === run.runId ? 'border-sky-300 bg-sky-50' : 'border-slate-200 bg-white hover:bg-slate-50'" @click="emit('select-run', run.runId)">
          ...
          <div v-if="runLogs.length === 0" class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-6 text-center text-sm text-slate-500">No session log yet.</div>
        </aside>
        ...
        <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
          <template v-if="selectedRun">
            ...
            <div class="mt-3">
              <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Latest Log Lines</p>
