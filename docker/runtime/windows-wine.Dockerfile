...
ENV CI=true \
    ...
    PATH=/pnpm:$PATH \
    WINEDEBUG=-all \
    STEAMCMD_LIVE_PROFILE=windows \
    STEAMCMD_LIVE_WINDOWS_WINE=1 \
...
RUN apt-get update \
  && apt-get install --no-install-recommends -y ca-certificates curl xauth xz-utils xvfb \
...
CMD ["pnpm", "run", "test:steamcmd:live"]
